...
public class DetailPlayerActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        musicID = BaseActivity.myBinder.getMusicList().get(BaseActivity.myBinder.getCurrentIndex()).getMusicID();
        if (getMusic(musicID).isCollected()){
            binding.playerPage.myLikeBtn.setImageResource(R.drawable.ic_like_selected);
        }
        else {
            binding.playerPage.myLikeBtn.setImageResource(R.drawable.ic_like);
        }
    }
    ...
    public void init() {
        ...
        binding.playerPage.myLikeBtn.setOnClickListener(this);
    }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.my_like_btn:
                ...
                if (!getMusic(musicID).isCollected()){
                    binding.playerPage.myLikeBtn.setImageResource(R.drawable.ic_like_selected);
                    StaticValue.shouCangNum++;
                    Intent intent = new Intent("collect");
                    localBroadcastManager.sendBroadcast(intent);
                    MusicResolver.collectMusicArrayList.add(getMusic(musicID));
                    getMusic(musicID).setCollected(true);
                }
                else {
                    binding.playerPage.myLikeBtn.setImageResource(R.drawable.ic_like);
                    MusicResolver.collectMusicArrayList.remove(getMusic(musicID));
                    StaticValue.shouCangNum--;
                    getMusic(musicID).setCollected(false);
                }
        }
    }
    ...
    private Music getMusic(String id) {
        Music music = null;
        for (Music m:MusicResolver.musicArrayList) {
